<template>
  <div class="w-full h-full flex flex-col gap-y-6 md:gap-y-8 overflow-y-auto">
    ...
    <div v-else class="flex flex-col gap-10">
      ...
      <div v-if="populatedGenres.length === 0"
    ...
    <div v-if="showAddDialog" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4"
      ...
      <div class="bg-surface-1 p-6 rounded-xl w-full max-w-sm flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-1">
          ...
          <input v-model="newGenre.name" type="text" placeholder="e.g., Thriller"
        ...
        <div class="flex flex-col gap-1">
          ...
          <textarea v-model="newGenre.description" rows="2" placeholder="Short description (optional)"
            class="px-3 py-2 rounded-lg border border-border bg-surface-2 text-text text-sm resize-none"></textarea>
        </div>
        <div class="flex gap-3">
          ...
          <button @click="addGenre" class="flex-1 px-4 py-2 rounded-lg bg-primary text-on-primary text-sm">
            Add
          </button>
        </div>
      </div>
