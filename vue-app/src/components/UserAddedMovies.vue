<template>
  <div class="w-full h-full p-0 flex flex-col gap-y-6 md:gap-y-8 rounded-xl">
    <!-- Toolbar -->
    <div class="flex-grow flex flex-col gap-y-3 p-2 sm:p-3 bg-surface-2 rounded-lg border border-border min-h-0">
      <div class="flex-shrink-0 flex flex-row items-center gap-x-2 md:gap-x-3">
        <!-- Global Search -->
        <InputText v-model="globalFilter" placeholder="Search your movie"
          class="w-full h-9 flex-grow !rounded-lg !bg-surface-3 !border-border" />

        <div class="flex-shrink-0 flex gap-x-2 md:gap-x-3">
          <!-- Add Movie -->
          <RouterLink :to="'/movies/add'"
            class="p-button h-9 flex items-center justify-center gap-x-2 rounded-lg !bg-surface-3 !border-border !text-text"
            @click="openAddDialog">
            <Plus :size="16" />
          ...
          <!-- Download Data -->
          <Button
            class="h-9 flex items-center justify-center gap-x-2 rounded-lg bg-surface-3! border-border! !text-text"
      ...
      <!-- DataTable -->
      <DataTable :value="movies" :filters="filters" :globalFilterFields="['title', 'description']" dataKey="id"
        paginator :rows="10" :loading="moviesStore.loading" class="p-datatable-gridlines rounded-xl! grow! min-h-0"
        ...
        <Column field="title" header="Title" sortable class="w-2xs">
          <template #body="{ data }">
            <div class="w-full flex gap-x-3">
              <RouterLink :to="`/movies/${data.uuid}`" class="underline">{{ data.title }}</RouterLink>
            </div>
          </template>
        </Column>
        ...
        <!-- Actions -->
        <Column header="Actions" headerStyle="width:160px; text-align:center" bodyStyle="text-align:center">
          <template #body="{ data }">
            <div class="w-full flex gap-x-3">
              <RouterLink :to="`/movies/${data.uuid}/edit`" class="p-button p-button-text p-button-sm">
                <Pencil :size="16" />
              </RouterLink>
              <Button icon="pi pi-trash" class="p-button-text p-button-sm p-button-danger"
                @click="confirmDelete(data.id)" title="Delete" />
            </div>
          </template>
        </Column>
      </DataTable>
